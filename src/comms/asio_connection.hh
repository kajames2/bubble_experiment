...
template <typename T>
class AsioConnection : public Connection<T> {
 public:
  AsioConnection(
      asio::io_context& context, asio::ip::tcp::socket socket,
      std::function<void(Message<T>)> do_handle_message,
 ...
 private:
  void WriteHeader() {
    asio::async_write(
        socket_,
        asio::buffer(&m_qMessagesOut.front().header_, sizeof(MessageHeader<T>)),
  ...
  void ReadHeader() {
    asio::async_read(
        socket_,
        asio::buffer(&m_msgTemporaryIn.header_, sizeof(MessageHeader<T>)),
        [this](std::error_code ec, std::size_t) {
          if (!ec) {
