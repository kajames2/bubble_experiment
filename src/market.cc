...
auto Market::GetBids() const -> std::vector<Bid> { return bids_; }

auto Market::GetStandingBid() const -> Bid {
  return *std::ranges::max_element(bids_);
}

auto Market::GetStandingAsk() const -> Ask {
  return *std::ranges::max_element(asks_);
}

auto Market::GetAsks() const -> std::vector<Ask> { return asks_; }
