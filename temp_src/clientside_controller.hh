...
#include "clientside_processor.hh"
...
class ClientsideController : public ServerMessageProcessor {
 public:
  ClientsideController(std::shared_ptr<AsioClient> client,
                       std::unique_ptr<ClientsideProcessor> proc)
  ...
  auto ReadPortfolio(std::string body) -> void {
    ...
    proc_->ProcessPortfolio(port.second);
  }
 ...
 private:
  ...
  std::unique_ptr<ClientsideProcessor> proc_;
};
