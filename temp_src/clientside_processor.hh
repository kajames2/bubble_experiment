#ifndef CLIENTSIDE_PROCESSOR_HH
#define CLIENTSIDE_PROCESSOR_HH
...
class ClientsideProcessor {
 public:
  ClientsideProcessor(std::shared_ptr<ClientState> state) : state_(state) {}
  ...
  auto ProcessAcceptedOffer(MarketSubmissionResult res) -> void {
    ...
    if (res.offer.player_id == state_->id) {
  ...
  auto ProcessPortfolio(Portfolio port) -> void {
    state_->port = port;
    OnPortfolioUpdated();
  }
  ...
  auto ProcessRoundStarted(RoundConfiguration conf) -> void {
    ...
    state_->exp_state.round = conf.round;
    state_->start_round_port = state_->port;
    OnQueuesUpdated();
    OnRoundStarted();
  }
  ...
  auto ProcessRoundEnded() -> void {
    state_->end_round_port = state_->port;
    OnRoundEnded();
  }
  auto ProcessReviewStarted() -> void {
    state_->post_dividend_port = state_->port;
    OnReviewStarted();
  }
  ...
  auto OnPortfolioUpdated() -> void {
    std::cout << PrettyPrint(state_->port) << '\n';
    ;
  }
  auto OnSubjectUpdated() -> void {}
  ...
  auto OnRoundStarted() -> void {
    std::cout << "Round " << state_->exp_state.round << " Started" << std::endl;
  }
